...
- `mini-ticket` (D-0523): signed service tickets — a receiver's attestation
  ...
  settlement layer admits it. Leaf crate, no value or governance edge.
...
|---|---|---|
| **1. Trustworthy tree** | Governance validator reads data not commentary (R1); green CI baseline (R2) | 2 done |
| **2. Money layer, auditable as one thing** | Retire the transparent payment path (R3); fees, change and multi-output (R4); chain-backed private ledger view (R5); amount disclosure (R6); decoy weights fitted to real traffic (R7) | 3 done, 1 ready, 1 blocked |
| **3. A network, not a protocol** | Consensus production gaps (R8); KEL freshness and witnesses (R9); BLE transport on real hardware (R10); a usable client (R11) | 2 active, 1 ready, 1 outside |
...
**20 items: 5 done, 2 active, 3 ready, 4 blocked, 6 outside.**
<!-- ROADMAP-SUMMARY-END -->
...
1. [`docs/FOUNDER_DIRECTIVES.md`](docs/FOUNDER_DIRECTIVES.md) — *why the
   project exists and what it values* — the one canonical eighteen-directive
   set (D-0090, extended D-0352); machine-readable as
   [`docs/CONSTITUTION_REGISTRY.json`](docs/CONSTITUTION_REGISTRY.json)
   (`FD-01`–`FD-18`, generated by `tools/constitution_registry.py`).
...
3. [`docs/DECISION_LOG.md`](docs/DECISION_LOG.md) — *why each choice was made,
   and when it was superseded* (append-only; main sequence `D-0001`–`D-0523`
