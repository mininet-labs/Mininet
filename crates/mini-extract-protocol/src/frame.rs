//! Length-delimited, size-bounded framing for the coordinator/worker IPC
//! channel (stdin/stdout of a spawned child process). Every message is
...
//! `mini-pipeline-protocol::{read_framed, write_framed}` even though the
//! wire shape is identical: each isolated-process protocol crate in this
//! tree owns its own tiny codec (the same choice `mini-intake-types`,
//! `mini-relay`, `mini-bridge`, and `mini-private-index` all made) rather
//! than creating a cross-domain dependency edge between unrelated
//! subsystems purely to save four lines of framing code.
