//! MiniSearch Track F1/F2/F3 (`docs/research/
...
//! observations (F1) and index segments (F2), plus deterministic merging
//! of per-provider query results while preserving provenance (F3).
//!
...
//! sources, it does not fetch anything over a network. No local
//! re-ranking of the merged list against a second, personalized profile
//! (Track F4). No provider payments (F5), private query transport (F6),
//! or historical snapshots (F7). This crate provides the wire format and
//! merge function two or more peers' results would need before any of
//! that can be built.
