...
//! derives indexing fields from that typed record, applies explicit local-memory
//! bounds, and keeps uncertainty visible: crawler timestamps are not canonical
//! time, missing digests are not changes, and equally timestamped providers may
//! disagree. See [`VersionRelation`].
//!
...
//! local view, not a signed shared history or a truth oracle.
...
pub use history::{
    ...
    DEFAULT_MAX_SNAPSHOTS_PER_URL, DEFAULT_MAX_SNAPSHOT_URLS, DEFAULT_MAX_TOTAL_SNAPSHOTS,
};
