...
//! and `mini_bounty::KeyImageLedger`, applied to `(payer, nonce)` instead of
//! `(device, nonce)`/a key image.
//!
...
//! different claims at the same nonce and showing each to a different
...
/// Tracks the first claim digest seen per `(payer, nonce)`, so a second,
...
pub trait ClaimWatcher {
    /// The digest already recorded for `(payer, nonce)`, if any.
    fn first_seen(&self, payer: &[u8], nonce: u64) -> Option<[u8; 32]>;

    /// Record `digest` as seen for `(payer, nonce)` if nothing was recorded
    ...
    fn observe(&mut self, payer: &[u8], nonce: u64, digest: [u8; 32]) -> bool;
}
...
impl ClaimWatcher for InMemoryClaimWatcher {
    fn first_seen(&self, payer: &[u8], nonce: u64) -> Option<[u8; 32]> {
        self.seen.get(&(payer.to_vec(), nonce)).copied()
    }

    fn observe(&mut self, payer: &[u8], nonce: u64, digest: [u8; 32]) -> bool {
        match self.seen.entry((payer.to_vec(), nonce)) {
...
mod tests {
    ...
    #[test]
    fn different_payers_or_different_nonces_never_conflict() {
        let mut w = InMemoryClaimWatcher::new();
        assert!(w.observe(b"payer-a", 0, [1u8; 32]));
        assert!(w.observe(b"payer-b", 0, [2u8; 32])); // different payer
        assert!(w.observe(b"payer-a", 1, [3u8; 32])); // different nonce
    }
}
