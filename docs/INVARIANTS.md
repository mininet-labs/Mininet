...
| M2 | **Offline/local payment is never final. It is a signed pending claim until canonical chain inclusion; wallets must distinguish pending / accepted / finalized.** | D5 | Founder review, D-0045 | ✅ `mini-settlement::SettlementState`/`WalletLabel` (D-0055) make the distinction a type — `is_final()` is true only for `Finalized`, never `AcceptedLocal`; `CanonicalLedgerView` now has a real, chain-backed implementation, `mini-execution::LedgerChain` (D-0061), closing [roadmap #41](../../issues/41)'s remaining gap — state only advances behind a verified `mini_chain::QuorumCertificate`, never speculatively ; D-0529 prototype: `mini-private-payment::account` separates pending/reserved/available and `reconcile` requires every input finalized (`tests/account.rs`); offline credit coupons remain receivables, not cash |
| M3 | **Canonical ordering alone decides conflicting spends (double-spends). No local committee, hotspot, relay, or cache may finalize ownership.** | D5, D4 | Founder review, D-0045 | ✅ `mini-settlement::reconcile` (D-0055) only ever finalizes a claim by reading a `CanonicalLedgerView`; test `conflicting_claims_at_the_same_sequence_never_both_finalize` proves exactly one of two conflicting claims resolves. `mini-execution::LedgerChain` (D-0061) is the real chain-backed `CanonicalLedgerView`, closing [roadmap #40](../../issues/40) — `tests/end_to_end.rs::two_independent_chains_fed_the_same_finalized_blocks_converge_to_identical_state` proves two honest nodes never disagree ; D-0529 prototype: `mini-economy::credit` deduplicates coupon slots and caps aggregate debt (`tests/credit.rs`); a canonical host remains required, and local sender journal commits do not finalize ownership |
...
| U1 | No forced auto-update / no off switch | D3, D2 | SPEC-00 P3 + SPEC-11 | ✅ `mini-update::AdoptionState` — `evaluate` never mutates state, `adopt` always re-verifies from scratch, `refuse` is a normal, unblocked outcome. `mini-installer::Installer::activate` (D-0071) — the one function that actually flips a running-software pointer — requires an explicit, caller-constructed `OwnerApproval` naming the exact release id (typed-domain rule); this crate never constructs one itself or self-invokes activation, and a failed health check auto-rolls-back to whatever was already running rather than forcing anything forward |
