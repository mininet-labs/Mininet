...
| Public web search | URL/search vocabulary, bounded crawler planning/fetch execution, static extraction, lexical index, ranker, query composition, signed segment exchange and local federation exist as separate libraries | Robots cache, durable frontier, sandbox process, end-to-end observation-to-index service, real peer transport, UI |
| Crawlers and indexers | Admission planner plus bounded HTTPS fetch runtime with redirect-by-redirect SSRF checks and explicit observations | Robots parser/cache, politeness scheduler, durable jobs, extraction sandbox, spam/malware pipeline and resource accounting |
