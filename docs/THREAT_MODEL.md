...
| **Timestamp manipulation** | `mini_chain::BlockHeader::timestamp_ms` was proposer-controlled and completely unchecked — a proposer could set any value, including flat or decreasing across heights, biasing any downstream logic that assumes monotonic block time (roadmap #44). | `mini_execution::LedgerChain::apply_finalized_block` now rejects a header whose `timestamp_ms` does not strictly exceed the previous finalized block's (D-0083); `mini-consensus`'s `validate_proposal` mirrors the same check at prevote time as a cheap early filter | ✅ Monotonicity enforced identically by every honest node (so it can never itself cause disagreement, Directive 4) — a real wall-clock bound (freshness vs. local time) is explicit unfinished follow-up, since no wall-clock semantics exist in this tree yet. |
| **Cross-domain signature replay** | `mini_chain::Vote`'s signed transcript had no domain-separation tag — every *other* signed transcript in this workspace (`mini-consensus`'s `Proposal`, `mini-settlement`'s `PaymentClaim`, `mini-bounty`'s claim) already prepends one, per `CLAUDE.md`'s typed-domain rule. A vote was the one place a signature over the wrong domain's bytes could in principle be replayed as a vote if the byte layouts ever collided (roadmap #44). | `VOTE_SIGN_DOMAIN` now prepended to `Vote::transcript` (D-0083) | ✅ Closed — same-workspace domain confusion is now structurally impossible; a genuine multi-chain/multi-network chain-id concept (for replay *across separate deployments* of this same protocol) remains unbuilt and is separate, larger follow-up work. |
| **Fee-mechanism manipulation** | `mini_value::fee::PriceHistory::add_entry` accepted a governed price of `0`, which would make every fee free regardless of the real-world value target — this crate has no opinion on *who* may call it (an external governance/authorization concern), but a zero price is never a sane value once a call is made (roadmap #44). | `PriceHistory::add_entry` now rejects `micro_mini_per_micro_cent == 0` (D-0083) | Partial — the zero-price bug is closed; a rate-limit/max-jump bound between consecutive prices is a genuine governance-policy question (how fast *should* the price be allowed to move), deliberately left to a founder decision rather than invented unilaterally. |
