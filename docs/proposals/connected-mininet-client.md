...
Verified: `cargo fmt`, strict Clippy and tests for `mini-desktop` and
`mini-sync`, including a real-TCP test where one host serves six session
connections (public sync, a shared private route, an unshared route
declined, idempotent repeat), and two live desktop instances on one Windows
11 machine exchanging a post and a follow through hosting + a session that
