...
export function simulate(p){
 ...
 for(let year=1;year<=p.years;year++){
  ...
  const actualHumans=Math.max(1,Math.round(p.humans*Math.pow(p.humanTarget/p.humans,Math.min(year,50)/50)));
